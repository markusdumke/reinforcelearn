...
qlearning_fa_experience_replay <- function(envir, makeFeatureVector, predict, train,
  ...
  k = 0
  features.state = matrix(0, nrow = batch.size, ncol = 70L) # vector("list", batch.size) # must be a list/array because one element could be a matrix
  target.Q = matrix(0, nrow = batch.size, ncol = envir$n.actions)
  ...
  for (i in seq_len(n.episodes)) {
    ...
    while (envir$episode.over == FALSE) {
      ...
      batch.states = sapply(batch, "[[", "state")
      batch.next.states = sapply(batch, "[[", "next.state")
      batch.actions = sapply(batch, "[[", "action")
      batch.rewards = sapply(batch, "[[", "reward")
      
      features.state = makeFeatureVector(batch.states)
      features.next.state = makeFeatureVector(batch.next.states)
