...
qnetwork <- function(envir, make_feature_vector, n.episodes = 10,
  ...
  optimizer = tf$train$GradientDescentOptimizer(learning_rate = learning.rate) # Gradient Descent Optimization
  updateModel = optimizer$minimize(loss) # minimize loss
  ...
  for (i in seq_len(n.episodes)) {
    ...
    while (envir$episode.over == FALSE) {
      ...
      W1 = sess$run(tuple(updateModel, W), feed_dict = dict(inputs = features.state, nextQ = target.Q))
