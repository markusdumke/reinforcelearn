# # REINFORCE
# reinforce = function(envir, n.episodes = 100, discount = 1, learning.rate = 0.01) {
#   
#   # start with random policy
#   policy = matrix(1 / envir$n.actions, nrow = envir$n.states, ncol = envir$n.actions)
#   # preferences for each action (table-lookup)
#   h = matrix(0, nrow = envir$n.states, ncol = envir$n.actions)
#   
#   for (i in seq_len(n.episodes)) {
#     states = c()
#     rewards = c()
#     actions = c()
#     envir$reset()
#     
#     while (envir$done == FALSE) {
#       s = envir$state
#       states = append(states, s)
#       action = sampleActionFromPolicy(policy[s + 1, ])
#       actions = append(actions, action)
#       envir$step(action)
#       rewards = append(rewards, envir$reward)
#       
#       if (envir$done) {
#         message(paste("Episode", i, "finished after", envir$n.steps, "steps."))
#         # update policy
#         for (i in seq_along(states)) {
#           G = sum(rewards[i:length(states)] * discount^(0:(length(states) - i)))
#           h[states[i], actions[i]] = h[states[i], actions[i]] + 
#             learning.rate * discount^(i - 1) * G * (1 - policy[states[i], actions[i]])
#         }
#         # softmax policy for discrete actions
#         policy = exp(h) / rowSums(exp(h))
#         #message(policy)
#         break
#       }
#     }
#   }
#   policy
# }
# 
# # REINFORCE + baseline
# reinforceBase = function(envir, n.episodes = 100, discount = 1, alpha = 0.01, beta = 0.1) {
#   
#   # start with random policy
#   policy = matrix(1 / envir$n.actions, nrow = envir$n.states, ncol = envir$n.actions)
#   # preferences for each action (table-lookup)
#   h = matrix(0, nrow = envir$n.states, ncol = envir$n.actions)
#   v = rep(0, envir$n.states)
#   
#   for (i in seq_len(n.episodes)) {
#     states = c()
#     rewards = c()
#     actions = c()
#     envir$reset()
#     
#     while (envir$done == FALSE) {
#       s = envir$state
#       states = append(states, s)
#       action = sampleActionFromPolicy(policy[s + 1, ])
#       actions = append(actions, action)
#       envir$step(action)
#       rewards = append(rewards, envir$reward)
#       
#       if (envir$done) {
#         message(paste("Episode", i, "finished after", envir$n.steps, "steps."))
#         # update policy
#         for (i in seq_along(states)) {
#           G = sum(rewards[i:length(states)] * discount^(0:(length(states) - i)))
#           delta = G - v[states[i] + 1]
#           v[states[i] + 1] = v[states[i] + 1] + beta * delta
#           h[states[i] + 1, ] = h[states[i] + 1, ] + alpha * discount^(i - 1) * 
#             delta * (makeNHot(actions[i] + 1, envir$n.actions) - policy[states[i] + 1, ])
#         }
#         # softmax policy for discrete actions
#         policy = exp(h) / rowSums(exp(h))
#         break
#       }
#     }
#   }
#   policy
# }
# 
# actorCritic = function(envir, n.episodes = 100, discount = 1, alpha = 0.01, beta = 0.1) {
#   
#   # start with random policy
#   policy = matrix(1 / envir$n.actions, nrow = envir$n.states, ncol = envir$n.actions)
#   # preferences for each action (table-lookup)
#   h = matrix(0, nrow = envir$n.states, ncol = envir$n.actions)
#   v = rep(0, envir$n.states)
#   
#   for (i in seq_len(n.episodes)) {
#     envir$reset()
#     j = 1
#     while(envir$done == FALSE) {
#       s = envir$state
#       action = sampleActionFromPolicy(policy[s + 1, ])
#       envir$step(action)
#       r = envir$reward
#       s_n = envir$state
#       
#       delta = r + discount * v[s_n + 1] - v[s + 1]
#       if (envir$done) {
#         delta = r - v[s + 1]
#       }
#       v[s + 1] = v[s + 1] + beta * delta
#       h[s + 1, ] = h[s + 1, ] + alpha * j * delta * (makeNHot(action + 1, envir$n.actions) - policy[s + 1, ])
#       # softmax policy for discrete actions
#       policy = exp(h) / rowSums(exp(h))
#       j = discount * j
#       if (envir$done) {
#         message(paste("Episode", i, "finished after", envir$n.steps, "steps."))
#         break
#       }
#     }
#   }
#   policy 
# }
# 
# actorCriticEligibility = function(envir, n.episodes = 100, discount = 1, alpha = 0.01, beta = 0.1, lambda = 0.5) {
#   
#   # start with random policy
#   policy = matrix(1 / envir$n.actions, nrow = envir$n.states, ncol = envir$n.actions)
#   # preferences for each action (table-lookup)
#   h = matrix(0, nrow = envir$n.states, ncol = envir$n.actions)
#   v = rep(0, envir$n.states)
#   
#   for (i in seq_len(n.episodes)) {
#     envir$reset()
#     e.policy = matrix(0, nrow = envir$n.states, ncol = envir$n.actions)
#     e.critic = rep(0, envir$n.states)
#     j = 1
#     while(envir$done == FALSE) {
#       s = envir$state
#       action = sampleActionFromPolicy(policy[s + 1, ])
#       envir$step(action)
#       r = envir$reward
#       s_n = envir$state
#       
#       delta = r + discount * v[s_n + 1] - v[s + 1]
#       if (envir$done) {
#         delta = r - v[s + 1]
#       }
#       e.critic[s + 1] = e.critic[s + 1, ] + j
#       e.critic = discount * lambda * e.critic
#       e.policy[s + 1, ] = e.policy[s + 1, ] + j * (makeNHot(action + 1, envir$n.actions) - policy[s + 1, ])
#       e.policy = discount * lambda * e.policy
#       
#       v = v + beta * delta * e.critic
#       h = h + alpha * j * delta * e.policy
#       # softmax policy for discrete actions
#       policy = exp(h) / rowSums(exp(h))
#       j = discount * j
#       if (envir$done) {
#         message(paste("Episode", i, "finished after", envir$n.steps, "steps."))
#         break
#       }
#     }
#   }
#   policy 
# }
# 
# 
# 
# # # Variant of cliff walking
# # library(reinforcelearn)
# # rewardFun = function(state, action, n.state) {
# #   if (n.state %in% 37:46) {
# #     return(- 100)
# #   } else {
# #     return(- 1)
# #   }
# # }
# # env = makeGridworld(shape = c(4, 12), goal.states = 47,
# #   cliff.states = 37:46, reward.step = - 1, reward.cliff = - 100,
# #   cliff.transition.done = TRUE, initial.state = 36, sampleReward = rewardFun)
# # 
# # actorCritic(env, n.episodes = 300)
# # actorCriticEligibility(env, n.episodes = 300, lambda = 0.5)
# # reinforceBase(env, n.episodes = 300, discount = 1)
# 
# # Q Actor Critic
# # Advantage Actor Critic
# # TD Actor Critic
# # TD lambda Actor Critic
