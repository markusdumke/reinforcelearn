...
The most well-known algorithm for reinforcement learning is probably Q-Learning [@watkins1989].

$Q$ is the action value function and specifies how good a state-action pair is. It is a matrix (number of states x number of actions). Interacting with the environment an action is chosen according to an $\epsilon$-greedy behavior policy
...
Then the $Q$ value of the current state-action pair is updated by
...
In `reinforcelearn` there is a `qlearning` function. In the following we will train on a simple navigation task, the windy gridworld. The first argument of all algorithms is called `envir`, where the environment can be passed on. The number of episodes can be specified via `n.episodes`.
...
The action value function will be initialized to 0. But you can also specify an initial value function via the `initial.value` argument.
...
The weights are then updated by gradient descent,
...
The Sarsa algorithm [@rummery1994] is similar to Q-Learning but samples the next action instead of taking the max. Therefore the update rule is
...
Expected Sarsa [@vanseijen2009] is a generalization of Q-Learning to arbitrary target policies. While Sarsa samples the next action, Expected Sarsa computes the expectation by multiplying each action value with the probability of choosing this action. The update rule is 
...
The Q($\sigma$) algorithm [@deasis2017] generalizes all the algorithms presented so far, i.e. Sarsa, Expected Sarsa and Q-Learning.
...
The factor $\psi$ controls the type of eligibility, for $\psi = 0$ this is the standard accumulating trace, for $\psi = 1$ this is the replacing trace [@eligibility]. The factor $\lambda$ is the eligibility decay parameter. The eligibility decrease is weighted with the target's policy probability of the next action for the Expected Sarsa part of the target.

We can specify $\psi$ via the `eligibility.type` argument and $\lambda$ via the `lambda` argument.
...
The idea of double learning [@hasselt2010] is to decouple action selection and action evaluation. For example in Q-Learning the update is 
...
Afterwards the eligibility traces are decayed by
...
An actor critic is a policy-based reinforcement learning algorithm [@sutton2017]. It uses a parametrized policy and value function. In `reinforcelearn` an advantage actor critic is implemented, which uses the td error of the state value function as a critic.
...
The actor critic can be used with eligibility traces, then there are separate eligibility traces for the policy parameters and the value function parameters, which can be decayed by different factor $\lambda$.
...
To solve the bandit, i.e. to find out, which action returns the most reward, we can use the `bandit` function. There are several different action selection methods implemented, e.g. `greedy`, `epsilon-greedy`, `UCB` and `gradient-bandit`.
...
The parameter `epsilon` can be decreased over time by passing a function `updateEpsilon`, which takes the current `epsilon` value and the number of episodes finished as arguments.
...
There is also a gradient-bandit algorithm [@sutton2017].
