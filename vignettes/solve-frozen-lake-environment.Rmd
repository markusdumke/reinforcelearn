...
Therefore we need to install python, gym and gym-http-api first.
...
Then we can start the environment from within R
...
res = qlearning(lake, n.episodes = 1000)
Q = res$Q
```

Then we can find the optimal policy by acting greedy with respect to $Q$.
...
```{r, eval = FALSE}
plot(x = 1:1000, y = res$rewards.per.episode / 1:1000, 
